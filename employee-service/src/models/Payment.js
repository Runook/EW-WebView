...
class Payment {
   ...
   * @returns {Promise<Object>} 付款记录列表和分页
   */
  static async getAll(filters = {}) {
    try {
      ...
      // 按订单过滤
      if (filters.order_id) {
        query = query.where('p.order_id', filters.order_id);
      }
      
      // 按客户过滤
      if (filters.customer_id) {
        query = query.where('p.customer_id', filters.customer_id);
      }
      
      // 按供应商过滤
      if (filters.vendor_id) {
        query = query.where('p.vendor_id', filters.vendor_id);
      }
   ...
   * @returns {Promise<Object>} 更新后的付款记录
   */
