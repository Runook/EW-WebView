...
class Order {
   ...
   */
  static async updateOrder(orderId, updateData, updatedBy, canEditAll = false) {
    const trx = await db.transaction();
    ...
    try {
      ...
      if (!canEditAll) {
        orderQuery = orderQuery.where(function() {
          this.where('created_by', updatedBy)
            .orWhere('assigned_to', updatedBy);
        });
      }
      
      const existingOrder = await orderQuery.first();
