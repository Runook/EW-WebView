...
router.get('/statistics', auth, requireEmployee, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '获取付款统计失败', error: error.message });
  }
});
...
router.get('/order/:orderId', auth, requireEmployee, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '获取订单付款记录失败', error: error.message });
  }
});
...
router.get('/:id', auth, requireEmployee, async (req, res) => {
  try {
    ...
    res.status(error.message === '付款记录不存在' ? 404 : 500).json({
...
router.post('/', auth, requireEmployee, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, data: payment, message: '付款记录创建成功' });
  } catch (error) {
...
router.put('/:id', auth, requireEmployee, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: payment, message: '付款记录更新成功' });
  } catch (error) {
